...
# WINDOWS EXE-BUNDLED VERSION
PNGQUANT_PATH               = '.\\bin\\pngquant\\pngquant.exe'
PNGNQ_PATH                  = '.\\bin\\pngnq\\pngnq.exe'
IMAGEMAGICK_CONVERT_PATH    = '.\\bin\\imagemagick-q8\\convert.exe'
IMAGEMAGICK_IDENTIFY_PATH   = '.\\bin\\imagemagick-q8\\identify.exe'
POTRACE_PATH                = '.\\bin\\potrace\\potrace.exe'

POTRACE_DPI = 72.0 # potrace docs say it's 72, but this seems to work best
...
VERSION = '1.00 (Windows exe-bundled)' #working on 1.00
...
def isolate_color(src, destlayer, target_color, palette, stack=False): #new version
    """fills the specified color of src with black, all else is white
...
def color_trace_multi(inputs, outputs, colors, quantization='mc', dither=None,
    remap=None, stack=False, prescale=1, despeckle=2, smoothcorners=1.0, optimizepaths=0.2):
    ...
    for i, o in zip(inputs, outputs):
        ...
        try:
            ...
            for i, color in enumerate(palette):
                ...
                isolate_color(this_reduced, this_layer, color, palette, stack=stack)
